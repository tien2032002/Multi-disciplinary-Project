<?php
    ...
    function updateStation($curID, $id, $name, $started_date, $capacity, $num_of_bikes, $address, $revenue, $status) {
        require('db.php');
        ...
        if ($curStation->id != $id) mysqli_query($con, "UPDATE employees
                                                        SET id='$id'
                                                        WHERE id=$curID");
        if ($curStation->name != $name) mysqli_query($con, "UPDATE employees
                                                        SET name='$name'
                                                        WHERE id=$curID");
        if ($curStation->started_date != $started_date) mysqli_query($con, "UPDATE employees
                                                        SET started_date='$started_date'
                                                        WHERE id=$curID");
        if ($curStation->capacity != $capacity) mysqli_query($con, "UPDATE employees
                                                        SET capacity='$capacity'
                                                        WHERE id=$curID");
        if ($curStation->num_of_bikes != $num_of_bikes) mysqli_query($con, "UPDATE employees
                                                        SET num_of_bikes='$num_of_bikes'
                                                        WHERE id=$curID");
        if ($curStation->address != $address) mysqli_query($con, "UPDATE employees
                                                        SET address='$address'
                                                        WHERE id=$curID"); 
        if ($curStation->revenue != $revenue) mysqli_query($con, "UPDATE employees
                                                        SET revenue='$revenue'
                                                        WHERE id=$curID");      
        if ($curStation->status != $status) mysqli_query($con, "UPDATE employees
                                                        SET status='$status'
                                                         WHERE id=$curID");                                            
    }

    function addStation($id, $name, $started_date, $capacity, $num_of_bikes, $address, $revenue, $status) {
        //add station to database
        require('db.php');
        $addStation = "INSERT INTO employees (id, name, started_date, capacity, num_of_bikes, address, revenue, status)
                   VALUE ('$id', '$name', '$started_date', '$capacity', '$num_of_bikes', '$address', '$revenue', '$status')";
        mysqli_query($con, $addStation);
    }

    function checkUpdateStation($curID, $id, $name, $started_date, $capacity, $num_of_bikes, $address, $revenue, $status) {
        ...
        if (strlen($num_of_bikes) == 0) $num_of_bikes = "invalid";
        ...
        //check revenue
        if (strlen($revenue) == 0) $revenueErr = "invalid";
        else $revenueErr = "good";
